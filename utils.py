    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        for i, j in enumerate(index_array):
            if self.filenames is None:
                ...
                image_path = self.filenames[j]
                ref_image = cv2.imread(self.filenames[j], is_color)
                image = ref_image.copy()
                if random.random() > 0.5:
                    gamma_list = [0.75, 1.25]
                    gamma = gamma_list[0 if random.random() < 0.5 else 1]
                    image = self.Color_Brightness.adjust_gamma(image, gamma)
